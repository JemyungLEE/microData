module EmissionDecomposer_hexa

# Developed date: 2. Sep. 2021
# Last modified date: 3. Sep. 2021
...
mutable struct factors
    ...
    cspf::Array{Float64, 3}     # Regional household expenditure profile: {Eora t-index, category, region}
    ...
    function factors(nr=0, nt=0, nc=0; f=Array{Float64,1}(), l=Array{Float64,2}(undef,0,0), p=Array{Float64,1}(), exp_pc=Array{Float64,1}(), exp_prof=Array{Float64,3}(undef,0,0,0), exp_cat=Array{Float64,2}(undef,0,0), de=Array{Float64,1}())
        if nr>0 && nt>0 && nc>0 && length(f)==0 && length(l)==0 && length(p)==0 && length(exp_pc)==0 && length(exp_prof)==0 && length(exp_cat) && length(de)==0
            new(zeros(nt), zeros(nt, nt), zeros(nr), zeros(nr), zeros(nt, nc, nr), zeros(nc, nr), zeros(nr))
        else new(f, l, p, exp_pc, exp_prof, exp_cat, de)
        end
    end
end
...
global mrio_tabs = Dict{Int, ee.tables}()                           # MRIO tables: {Year, MRIO tables (t, v ,y , q)}# global conc_mat_conv = Dict{Int, Dict{String, Array{Float64, 2}}}() # Base-year price converted concordance matrix: {year, {nation, {Eora sectors, Nation sectors}}}
...
function importData(; hh_data::Module, mrio_data::Module, cat_data::Module, nations = [])
    ...
    global nt_wgh, di_emiss, sc_cat = cat_data.wghNuts, cat_data.directCE, cat_data.cat
    ...
    sc_yrs, ct_yrs = sort(collect(keys(sc_list))), sort(collect(keys(cat_data.sec)))
    if sc_yrs == ct_yrs; for y in sc_yrs; if sc_list[y] != cat_data.sec[y]; println(y, " mdata and cdata sectors are different.") end end
    else print("micro_data and cateory_data have different YEAR reference.")
    end
end
...
function decomposeFactors(year, baseYear, nation = ""; visible = false, pop_nuts3 = true, pop_dens = 0)
    ...
    # f * L * p * tot_ce_pc * [con * hbs_profile_by_category * hbs_proportion_by_category] + DE
    ...
    for y in year, n in nats
        ...
        nr, nt, nc, ns = length(nts), size(mrio.t, 1), length(cat_list), length(scl)
        ft_p, ft_cepc, ft_cspf, ft_cpbc, ft_de = zeros(nr), zeros(nr), zeros(nt, nc, nr), zeros(nc, nr), zeros(nr)
        ...
        ft.p, ft.cepc, ft.cspf, ft.cpbc, ft.de = ft_p, ft_cepc, ft_cspf, ft_cpbc, ft_de
...
function calculateDeltaFactors(target_year, base_year, nation, delta_factor, sub_list)
    ...
    var = [fts[subs[1]].f, fts[subs[2]].l, fts[subs[3]].p, fts[subs[4]].cepc, fts[subs[5]].cspf, fts[subs[6]].cpbc]
    var[delta_factor] = dltByNat[nation][delta_factor]

    nt, nr = size(var[5], 1), size(var[5], 3)
    cspf = zeros(Float64, nt, nr)
    for i = 1:nr; cspf[:,i] = var[5][:,:,i] * var[6][:,i] end

    return sum(var[1] .* var[2] * ((var[3] .* var[4])' .* cspf), dims=1)'
end

function calculateEmission(year, nation)
    global sda_factors
    ft = sda_factors[year][nation]
    nt, nr = size(ft.cspf, 1), length(nutsByNat[year][nation])
    ft_cspf = zeros(Float64, nt, nr)
    for i = 1:nr; ft_cspf[:,i] = ft.cspf[:,:,i] * ft.cpbc[:,i] end

    return sum(ft.f .* ft.l * ((ft.p .* ft.cepc)' .* ft_cspf), dims=1)'
end
...
function printFactors(outputPath; year=0, nation="")
    ...
    global sda_factors, l_factor, nutsByNat, cat_list
    ...
    for y in yrs
        ...
        for n in nats
            ...
            if nt == size(ft.cspf,1) && nr == size(ft.cspf,2)
                for i = 1:nr
                    println(f, nutsByNat[n][i])
                    for j = 1:nc; print(f, ft.cspf[i,1,j]); for k = 2:nt; print(f, "\t", ft.cspf[i,k,j]) end; println(f) end
                end
            else println(n, ": CSPF matrix and F & P vectors sizes do not match.")
            end
            println(f, "[cpbc]")
            for i = 1:nr; print(f, ft.cpbc[i,1]); for j = 2:nc; ; print(f, "\t", ft.cpbc[i,j]) end; println(f) end
            println(f, "[de]")
...
function printDelta(outputFile; cf_print = true, st_print = true)
    ...
    print(f, vs, "f", vs, "L", vs, "p", vs, "exp_per_cap", vs, "exp_profile", vs, "exp_cat", vs, "de", vs, "total_delta")
...
function testSDA(year)
    ...
    for n in nat_list, nt in nutsBuNat[year][n]
        ...
        cf[n] = sum(ft.f .* ft.l * ((ft.p .* ft.cepc)' .* ft.cspf * ft.cpbc), dims=1)' + ft.de
    end
    ...
    for n in nat_list
        ...
        println(n, "\t", sda_factors[year][n].cpbc)
    end
